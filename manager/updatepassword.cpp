    ...
    ui(new Ui::UpdatepassWord)
{
    ...
    ui->NewPwdCofrmlineedit->setEnabled(false);
    connect(ui->OldPwdlineedit, &QLineEdit::textChanged, this, &UpdatepassWord::textChanged_check);
}
...
void UpdatepassWord::textChanged_check(const QString &text)
{
    QSqlQuery query;
    ...
    if (query.value(0).toString() == text)
    {
        ...
        ui->NewPwdCofrmlineedit->setEnabled(true);
...
{
    if (!ui->NewPwdlineedit->text().isEmpty() && !ui->NewPwdCofrmlineedit->text().isEmpty())
    {
        if (ui->NewPwdlineedit->text() == ui->NewPwdCofrmlineedit->text())
        {
